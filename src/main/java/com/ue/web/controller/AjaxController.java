...
import com.ue.web.model.AjaxResponseBody;
import com.ue.web.model.SearchCriteria;
import com.ue.web.model.User;
...
public class AjaxController {
	...
	public AjaxResponseBody getSearchResultViaAjax(@RequestBody SearchCriteria search, HttpServletRequest request, HttpServletResponse response) throws JsonProcessingException {
		Map<String, Cookie> cookies = ReadCookieMap(request);
		Cookie cookie = cookies.get("token");
		if (cookie != null) {
			String token = cookie.getValue();
			Claims body = Jwts.parser().setSigningKey(WelcomeController.generateKey("secret")).parseClaimsJws(token).getBody();
